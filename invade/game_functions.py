...
def update_bullets(settings, screen, ship, aliens, bullets):
    ...
    check_bullet_aliens_collisions(settings, screen, ship, aliens, bullets)


def check_bullet_aliens_collisions(settings, screen, ship, aliens, bullets):
    '''检查是否有子弹击中外星人'''
    # 如果是这样，就删除相应的子弹和外星人
...
def update_aliens(settings, start, screen, ship, aliens, bullets):
    '''检查是否有外星人位于边缘'''
    ...
    if pygame.sprite.spritecollideany(ship, aliens):
        ship_hit(settings, start, screen, ship, aliens, bullets)

    # 检测外星飞船是否到底部
    check_aliens_bottom(settings, start, screen, ship, aliens, bullets)


def check_aliens_bottom(settings, start, screen, ship, aliens, bullets):
    screen_rect = screen.get_rect()
    for alien in aliens.sprites():
        if alien.rect.bottom >= screen_rect.bottom:
            "像飞机被撞一样处理"
            ship_hit(settings, start, screen, ship, alien, bullets)
            break
        

def ship_hit(settings, start, screen, ship, aliens, bullets):
    '''相应被外星人撞到的飞船'''
